...
transform_and_plot(nc, new_crs = "EPSG:32113", title = "New Mexico")
...
- I think it would be slightly off.
...
st_crs(mn_cities)
st_crs(mn_water)
st_crs(mn_counties) # this one is epsg
...
ggplot() +
    ...
    labs(title = "MN City Population >= 10,000") + 
    ggthemes::theme_map() +
...
- Read the function description at the top. What is the advantage of using this function for making color palettes?
  - They are uniform in both color and black and white.
  - It allows for effective and accessible data viz. The colors are color-blind friendly.
- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?
  - d is for discrete (categorical), c is for continuous (numerical), and b is for binned.
...
png("../figures/tc_map_zoom.png", width = 600, height = 400)
