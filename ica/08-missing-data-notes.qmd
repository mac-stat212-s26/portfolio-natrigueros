...
1. **Missing data mechanism** For each of the following situations, propose what missing data mechanism you think is most likely at play. 
  ...
  - MCar if its technical issues, MAR if its associated with other variables.
  ...
  - MCAR if the issue affects the platform evenly. MAR if affected differently but there are identified groups.
...
- Ozone missing when solar is at higher values for both situations.
- In version 1, we have observed data in the Solar range for missing Ozone values, so prediction would be possible.
- In version 2, Ozone values are missing when Solar extrapolates from observed data, so we don't get a grasp of their relationship.
...
- MAR/MNAR of ozone is probably related to Month (june). Ozone's value is higher in July and August, lower with higher winds, and higher with higher temperature.
...
```{r}
ggplot(airquality, aes(fill = is.na(Ozone), x = factor(Month))) + geom_bar(position = "fill")
